...
// ---- View 1: Mastery XP ------------------------------------------------
export function computeMasteryXp(entries) {
    const totals = {};
...
export function groupedReasonsByTrack(entries) {
    const byTrack = {};
    for (const entry of entries) {
        for (const delta of entry.xpDeltas) {
            const map = byTrack[delta.track] ?? new Map();
...
export function computeIdentityAxes(entries) {
    const totals = {};
    for (const entry of entries) {
...
// as an accusation.
export function deriveBookLabels(axisTotals) {
    const labels = [];
    const get = (a) => axisTotals[a] ?? 0;
    if (get("player_advocacy") >= 6 && get("org_alignment") >= 6 && get("health_stewardship") >= 6) {
        labels.push("Balanced Builder");
    }
    if (get("player_advocacy") >= 10)
        labels.push("Player Advocate");
    if (get("org_alignment") >= 10)
        labels.push("Organization Soldier");
    if (get("health_stewardship") >= 10)
        labels.push("Health Steward");
    if (get("competitive_drive") >= 10)
        labels.push("Win-Now Operator");
    if (get("clubhouse_authority") >= 10)
        labels.push("Standards Setter");
    if (get("org_alignment") >= 12 && get("player_advocacy") <= -4)
        labels.push("Company Man");
    if (get("competitive_drive") >= 10 && get("health_stewardship") <= -5)
        labels.push("Reckless Competitor");
    if (labels.length === 0)
        labels.push("Still Finding His Footing");
    return labels;
}
const EARLY_READ_PHRASES = {
...
export function deriveEarlyRead(axisTotals) {
    const entries = Object.entries(axisTotals);
    if (entries.length === 0)
        return null;
    const [topAxis, topValue] = entries.reduce((best, cur) => (Math.abs(cur[1]) > Math.abs(best[1]) ? cur : best));
    if (Math.abs(topValue) < 4)
        return null; // too faint to call yet
    return EARLY_READ_PHRASES[topAxis];
}
export function computeRelationshipTrust(entries) {
