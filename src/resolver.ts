...
const FRONT_OFFICE = { targetId: "front_office_farm_director", targetName: "Farm Director", audience: "front_office" as const };
const ALVAREZ_REL = { targetId: "player_alvarez", targetName: "Mateo Alvarez", audience: "specific_player" as const };
const COBB_REL = { targetId: "player_cobb", targetName: "Ray Cobb", audience: "specific_player" as const };
const ROSA_REL = { targetId: "staff_rosa_mercado", targetName: "Rosa Mercado", audience: "staff" as const };
const PRICE_REL = { targetId: "player_jonah_price", targetName: "Jonah Price", audience: "specific_player" as const };
const TRAINER_REL = { targetId: "staff_trainer", targetName: "Trainer", audience: "staff" as const };

// Shape shared by every situation's choice set, not just the lineup one --
// the name stuck from the first slice but this now covers any day where
// the player picks A/B/C/D and the pick writes one ledger entry.
export interface LineupChoice {
  id: LineupChoiceId;
  label: string;
  assistantRecommended: boolean;
  alvarezStarts: boolean;
  strengthModifier: number; // applied to team strength for this game
  fatigueDeltas: Record<string, number>; // playerId -> delta
  moraleDeltas: Record<string, number>;
  developmentDeltas: Record<string, number>;
  notes: string[]; // game-flavor text, shown alongside the score
  overridePitcherId?: string; // set when the choice swaps who actually starts
  // Everything below feeds one DecisionLedgerEntry.
  tags: DecisionTag[];
...
function withoutZeros(deltas: AxisDelta[]): AxisDelta[] {
...
export function buildLineupOptions(alvarez: Player, directive: Directive): LineupChoice[] {
  ...
  const options: LineupChoice[] = [
    {
      ...
      tags: ["org_directive", "priority_prospect", "lineup", ...(alvarezTired ? (["fatigue"] as DecisionTag[]) : [])],
      ...
      xpDeltas: [
        ...
        ...(alvarezTired
          ? ([{ track: "health_management", amount: -4, reason: "Played a fatigued 19-year-old in a full-workload spot." }] as XpDelta[])
          : []),
      ],
      relationshipDeltas: [
    ...
    },
    {
      ...
      tags: ["fatigue", "priority_prospect", ...(directiveOpen ? (["org_directive"] as DecisionTag[]) : [])],
      axisDeltas: withoutZeros([
    ...
    },
    {
      ...
      tags: ["veteran_playing_time", "lineup", "winning_now", ...(directiveOpen ? (["org_directive"] as DecisionTag[]) : [])],
      axisDeltas: withoutZeros([
...
export function computeBaseTeamStrength(hitters: Player[], startingPitcher: Player): number {
...
export function resolveGame(opponent: Opponent, teamStrength: number, rng: () => number = Math.random): GameOutcome {
...
export function maybePitcherNote(pitcher: Player, rng: () => number = Math.random): string | null {
...
export function buildInjuredPitcherOptions(price: Player): LineupChoice[] {
  const options: LineupChoice[] = [
    {
      id: "A",
...
export function maybeInjuryFlareNote(rng: () => number = Math.random): string | null {
  if (rng() < 0.3) {
