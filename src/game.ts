...
import type { DecisionLedgerEntry, GameResult, Player } from "./types.js";
import {
  buildInjuredPitcherOptions,
  buildLineupOptions,
  computeBaseTeamStrength,
  maybeInjuryFlareNote,
  maybePitcherNote,
  resolveGame,
} from "./resolver.js";
import type { LineupChoice } from "./resolver.js";
...
function cloneRoster(): Player[] {
  return team.roster.map((p) => ({ ...p }));
}

function lineupIdsForChoice(choiceId: string): string[] {
  if (choiceId === "C") return BASE_STARTER_IDS.map((id) => (id === "alvarez" ? "boyd" : id));
  if (choiceId === "D") return BASE_STARTER_IDS.map((id) => (id === "alvarez" ? "cobb" : id));
  return BASE_STARTER_IDS;
}

function findPlayer(roster: Player[], id: string): Player {
  const p = roster.find((pl) => pl.id === id);
  if (!p) throw new Error(`Unknown player id: ${id}`);
  return p;
}

function applyDeltas(roster: Player[], deltas: Record<string, number>, field: "fatigue" | "morale" | "development") {
...
function recommendationLine(opt: LineupChoice, alvarez: Player): string {
  if (opt.id === "C") return `Alvarez is running hot on fatigue (${alvarez.fatigue}/100) -- I'd sit him today.`;
  if (opt.id === "B") return "Start him, but bat him eighth and keep an eye on tomorrow.";
  return "Ready when you are.";
}

function trainerNote(opt: LineupChoice): string {
  if (opt.id === "B") {
...
async function promptChoice(reader: LineReader, validIds: string[]): Promise<string> {
  for (;;) {
    const answer = (await reader.ask(`Choice [${validIds.join("/")}]: `)).trim().toUpperCase();
    if (validIds.includes(answer)) return answer;
    console.log(`Please enter one of: ${validIds.join(", ")}`);
  }
}

async function playWeek(reader: LineReader): Promise<void> {
  ...
  for (const day of schedule) {
    ...
    let chosen: LineupChoice;
    let effectivePitcherId: string;
    let hitters: Player[];
    ...
    if (isInjuryDay) {
      ...
      console.log(`Opponent: ${day.opponent.name}`);
      console.log(
        `Org Directive: ${directive.title} -- ${directive.startsSoFar}/${directive.requiredStarts} starts so far, ${directive.gamesRemaining} game${
          directive.gamesRemaining === 1 ? "" : "s"
        } left this week.`
      );
      ...
      directive.gamesRemaining = Math.max(0, directive.gamesRemaining - 1);
      if (chosen.alvarezStarts) directive.startsSoFar += 1;
  ...
  console.log(
    formatWeeklyEvaluation(
      { wins, losses },
      ledger,
      { title: directive.title, startsSoFar: directive.startsSoFar, requiredStarts: directive.requiredStarts }
    )
  );
}
