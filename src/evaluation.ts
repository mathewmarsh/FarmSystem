import type { DecisionLedgerEntry, MasteryTrack } from "./types.js";
import {
  computeIdentityAxes,
  computeMasteryXp,
  computeRelationshipTrust,
  deriveBookLabels,
  deriveEarlyRead,
  groupedReasonsByTrack,
} from "./ledger.js";

const ACTIVE_TRACKS: MasteryTrack[] = ["player_development", "health_management", "organizational_trust"];

const TRACK_LABEL: Record<MasteryTrack, string> = {
...
export function formatWeeklyEvaluation(
  ...
  if (bookLabels.length === 1 && bookLabels[0] === "Still Finding His Footing") {
    ...
    if (earlyRead) lines.push(`  Early Read: ${earlyRead}`);
  }
  ...
  for (const rel of trust) {
    const parts: string[] = [];
    if (rel.trust !== 0) parts.push(`trust ${rel.trust >= 0 ? "+" : ""}${rel.trust}`);
    if (rel.respect !== 0) parts.push(`respect ${rel.respect >= 0 ? "+" : ""}${rel.respect}`);
    if (rel.morale !== 0) parts.push(`morale ${rel.morale >= 0 ? "+" : ""}${rel.morale}`);
...
function formatDirectiveCompletion(summary: DirectiveSummary): string {
